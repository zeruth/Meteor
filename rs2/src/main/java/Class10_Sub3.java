public final class Class10_Sub3 extends Linkable {
   private boolean aBoolean133 = false;
   public Class5[] aClass5Array3 = new Class5[5];
   public Class3 aClass3_1;
   public Class11 aClass11_1;
   public Class36 aClass36_1;
   public Class29 aClass29_1;
   public Class45 aClass45_1;
   public int anInt548;
   public int[] anIntArray162 = new int[5];
   public Class21 aClass21_1;
   public boolean aBoolean135;
   public int anInt544;
   public int anInt550;
   public int anInt545;
   public int anInt547;
   public int anInt546;
   public Class10_Sub3 aClass10_Sub3_1;
   public int anInt549;
   public boolean aBoolean134;
   public int anInt551;
   public boolean aBoolean136;
   public int anInt552;
   public int anInt554;
   public int anInt553;

   public Class10_Sub3(int var1, int var2, int var3) {
      this.anInt547 = this.anInt544 = var1;
      this.anInt545 = var2;
      this.anInt546 = var3;
   }
}
